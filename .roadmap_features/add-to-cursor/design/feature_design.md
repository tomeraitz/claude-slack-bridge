...
The integration method is intentionally identical to Claude Code — same `docker exec` transport, same `session.py` entry point, same env-var config model.
...
No new libraries, services, or runtimes are introduced. No code changes are required — only documentation.
...
No daemon, session, or MCP server source files are modified.
