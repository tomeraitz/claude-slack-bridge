...
- **Live progress, no timeout** — a run has no wall-clock limit; it works until it finishes or you stop it. While it runs, the bot edits one status message in the thread: recent actions (newest first, up to 100/update), a collapsible **changed-files** section with per-file `+/−` churn, a live todo checklist, and a muted resource line (per-tool breakdown, subagents, errors, tokens in/out/cache, context used/window, turns, model, skills used, MCP servers) — all derived from the CLI's event stream with no extra model calls — then collapses it to a one-line summary. A stuck run (no activity for `IDLE_TIMEOUT_SECONDS`) is killed automatically. Turn the live message off with `LIVE_PROGRESS=false` (stop then falls back to the 🛑 reaction).
